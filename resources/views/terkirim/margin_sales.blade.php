                ...
                <div class="card-body">
                    ...
                    <table class="table table-hover table-bordered table-stripped" id="example2">
                        ...
                        @foreach($order_details as $key => $order_details)   
                                <td>- {{ str_replace('_', ' ', $order_details->keranjang->nama_produk) }} {{$order_details->jumlah}}btl ( {{ number_format($order_details->keranjang->harga_produk)}}/btl ) | {{ number_format($order_details->jumlah_harga)}} | *Margin : {{ number_format($order_details->margin_produk_sales)}}*</td>
                            </tr>
                        @endforeach
                        <td><strong>*Total Tagihan Hari ini : Rp. {{ number_format($order->jumlah_harga)}}*</strong><br><strong>*Total Sisa : Rp. {{ number_format($order->total_margin_sales)}}*</strong></td>
                        </tbody>
                    </table>
